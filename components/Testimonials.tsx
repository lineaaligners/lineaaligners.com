...
export const Testimonials: React.FC<{ language: 'en' | 'sq' }> = ({ language }) => {
  ...
  return (
    <section className="py-24 bg-[#193D6D] overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          <h2 className="text-[#87CEEB] font-bold uppercase tracking-wider text-sm">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {testimonials.map((t, idx) => (
            <div 
              key={idx} 
              className="bg-[#142A4D] p-8 rounded-[32px] border border-white/5 flex flex-col justify-between transition-all hover:shadow-2xl hover:shadow-blue-900/40 hover:-translate-y-1 hover:border-[#4169E1]/30"
            >
              <div className="space-y-4">
                <div className="flex gap-1 text-[#87CEEB]">
              ...
              <div className="mt-8 flex items-center gap-4">
                <img 
                  src={t.image} 
                  alt={t.name} 
                  className="w-16 h-16 rounded-full border-2 border-white/20 object-cover shadow-sm"
                  loading="lazy"
                  width="64"
                  height="64"
                />
